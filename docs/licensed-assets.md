...
## Canonical credits record

The player-facing inventory of dependencies, licenses, usage terms, and attributions is
maintained in
`LocalPackages/com.budgetgamedev.game.brocoli/Resources/Brocoli/Credits.txt` and displayed
by BROcoli's Credits menu. Update that record whenever restricted content is acquired,
updated, or removed instead of duplicating the inventory here. The encrypted JSON
sidecars remain the machine-readable acquisition and integrity records for each payload.
