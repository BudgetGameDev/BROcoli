...
async function dispatchBuildFetch(harness, filename = "game.wasm") {
  ...
  assert.ok(
    responsePromise,
    "Build requests must be handled by the service worker",
  );
...
(async () => {
  ...
  assert.equal(
    network.fetchCalls[0].options,
    undefined,
    "Build fetches must not carry an abort signal",
  );
  assert.equal(
    network.abortControllers,
    0,
    "Large Build downloads must not create a timeout controller",
  );
  assert.equal(
    network.timers.length,
    0,
    "Large Build downloads must not have a fixed timeout",
  );
  assert.equal(
    network.cacheWrites.length,
    1,
    "Successful Build downloads must be cached",
  );
  ...
  assert.strictEqual(
    await dispatchBuildFetch(cached, "game.data"),
    cachedResponse,
  );
  assert.equal(
    cached.fetchCalls.length,
    0,
    "Cached Build files must not be downloaded again",
  );
  ...
  assert.equal(
    fallback.status,
    503,
    "An uncached offline Build request must fail explicitly",
  );
