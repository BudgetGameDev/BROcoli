...
assert.ok(
  pipelineGuid,
  "GraphicsSettings must name the active render pipeline asset",
);
const pipelinePath = findAssetByGuid(pipelineGuid[1]);
assert.ok(
  pipelinePath,
  `No asset matches the active render pipeline GUID ${pipelineGuid[1]}`,
);
...
assert.ok(
  resizeFunction,
  "updateCanvasSize must remain present in the WebGL template",
);
...
assert.match(versionChecker, /Local build, skipping hosted version check/);
assert.match(serviceWorker, /Local build, skipping hosted version check/);
assert.doesNotMatch(
  iosOptimizer,
...
const additionalLights = urpSettings.match(
  /^\s*m_AdditionalLightsPerObjectLimit:\s*(\d+)$/m,
);
assert.ok(webglQuality, "WebGL must declare a default quality profile");
assert.ok(
  Number(webglQuality[1]) >= 3,
  "WebGL must retain the High-or-better lighting profile",
);
