...
function readInitialMemoryPages(wasmPath) {
  ...
  while (cursor.offset < bytes.length) {
    ...
    assert.ok(
      ...
      "WASM section extends beyond the file",
    );
    ...
    if (sectionId === 5) {
      ...
      assert.equal(
        memoryCount,
        1,
        "Unity player must declare exactly one WASM memory",
      );
      ...
      if ((flags & 1) !== 0) {
        ...
        assert.ok(
          ...
          "WASM maximum memory is below its initial memory",
        );
      }
      return initialPages;
    }
...
assert.ok(
  fs.existsSync(path.join(buildDirectory, "index.html")),
  "WebGL build is missing index.html",
);
assert.ok(
  fs.existsSync(buildFilesDirectory),
  "WebGL build is missing its Build directory",
);
assert.equal(
  findFiles(buildFilesDirectory, ".loader.js").length,
  1,
  "Expected one Unity loader",
);
...
assert.equal(
  wasmFiles.length,
  1,
  "Expected one uncompressed Unity WASM module",
);
...
console.log(
  `webgl-build: compiled WASM starts with ${initialMemoryBytes / 1048576} MiB`,
);
