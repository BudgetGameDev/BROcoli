...
function environment({
  ...
  if (touch) {
    browserWindow.ontouchstart = null;
  }
...
for (const maxTouchPoints of [0, 5]) {
  ...
  assert.equal(
    result.isAppleMobile,
    false,
    "macOS Safari must not be detected as iOS",
  );
  assert.equal(
    result.isMobile,
    false,
    "macOS Safari must retain desktop controls",
  );
  assert.equal(
    result.unityDevicePixelRatio,
    2,
    "macOS Safari must retain Retina rendering",
  );
}

const ipad = platform.detect(
  environment({
...
assert.equal(
  ipad.unityDevicePixelRatio,
  2,
  "iPadOS must retain capped Retina rendering",
);

const iphone = platform.detect(
  environment({
    userAgent:
...
assert.equal(
  iphone.unityDevicePixelRatio,
  2,
  "iPhone Safari must use capped Retina rendering",
);

const iosChrome = platform.detect(
  environment({
    userAgent:
      "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) " +
...
assert.equal(
  iosChrome.unityDevicePixelRatio,
  2,
  "iOS Chrome shares the capped Retina policy",
);

const android = platform.detect(
  environment({
    userAgent:
...
const windowsChrome = platform.detect(
  environment({
    userAgent:
...
const macChrome = platform.detect(
  environment({
    userAgent:
