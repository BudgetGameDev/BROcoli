...
namespace BudgetGameDev.Games.Brocoli.Tests
{
    ...
    public sealed class RunProgressionTests
    {
        ...
        public void ItTimesEachLevelAndCountsTheKillsThatBoughtIt()
        {
            ...
            progression.Sample(0f, 1f, 100f, 100f, 0);
            progression.Sample(30f, 2f, 80f, 100f, 6);
            progression.Sample(90f, 3f, 60f, 100f, 20);
        ...
        public void LevelsThatLandInTheSameSampleShareTheGapRatherThanArrivingFree()
        {
            var progression = new RunProgression();
            progression.Sample(0f, 1f, 100f, 100f, 0);
            progression.Sample(40f, 3f, 100f, 100f, 10);
        ...
        public void ANewLifeRestartsTheLevelClockInsteadOfBillingItForTheLastOne()
        {
            var progression = new RunProgression();
            progression.Sample(0f, 1f, 100f, 100f, 0);
            progression.Sample(60f, 2f, 20f, 100f, 8);
            progression.NoteDeath();
            progression.Sample(120f, 0f, 0f, 0f, 8);
            progression.Sample(150f, 2f, 90f, 100f, 12);
        ...
        public void ItSummarisesPressureAsMeanHealthTheWorstMomentAndTimeInDanger()
        {
            var progression = new RunProgression();
            progression.Sample(0f, 1f, 100f, 100f, 0);
            progression.Sample(10f, 1f, 20f, 100f, 1);
            progression.Sample(20f, 1f, 60f, 100f, 2);
            progression.Sample(30f, 1f, 100f, 100f, 3);
        ...
        public void EarlyAndLateLevelsAreAveragedSeparatelySoTheCurveShapeIsVisible()
        {
            ...
            float time = 0f;
            int kills = 0;
            for (int level = 2; level <= ProgressionBalance.EarlyLevels; level++)
            {
                time += 20f;
                kills += 4;
                progression.Sample(time, level, 100f, 100f, kills);
            }
            for (int level = ProgressionBalance.EarlyLevels + 1; level <= 8; level++)
            {
                time += 80f;
                kills += 20;
                progression.Sample(time, level, 100f, 100f, kills);
