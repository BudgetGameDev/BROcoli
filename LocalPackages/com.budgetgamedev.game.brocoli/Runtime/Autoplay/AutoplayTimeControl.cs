...
namespace BudgetGameDev.Games.Brocoli
{
    ...
    /// </summary>
    internal static class AutoplayTimeControl
    {
        /// <summary>Below this a frame advances so little that nothing is gained.</summary>
        internal const float MinimumStep = 1f / 240f;

        /// <summary>Physics sub-steps one rendered frame may cover.</summary>
        internal const int MaximumPhysicsStepsPerFrame = 4;

        internal static float ResolveCaptureStep(float requested, float fixedStep)
        {
        ...
        internal static float GameDelta =>
            Time.captureDeltaTime > 0f ? Time.captureDeltaTime : Time.unscaledDeltaTime;

        /// <summary>Game-seconds simulated per wall-clock second; 0 before any elapse.</summary>
        internal static float Speedup(float gameSeconds, float realSeconds) =>
