...
namespace BudgetGameDev.Games.Brocoli
{
    ...
    internal static class BotDecisionPolicy
    {
        ...
        )
        {
            ...
            return UtilitySelection.Choose(
                Scored,
                new ObservedPolicy(situation, tuning),
                previous,
                Hysteresis,
                BotIntent.Explore
            );
        ...
        private readonly struct ObservedPolicy : IUtilityPolicy<BotIntent>
        {
            private readonly BotSituation situation;
            private readonly BotTuning tuning;

            internal ObservedPolicy(BotSituation situation, BotTuning tuning)
            {
                this.situation = situation;
                this.tuning = tuning;
            }

            public float Score(BotIntent action) => Utility(action, situation, tuning);
        }
        ...
        private static float RetreatUtility(BotSituation situation, BotTuning tuning)
        {
            ...
            // Once a fight stops making progress, leave through a planned route.
            // Low health alone must not keep the agent circling at a comfortable
            // distance forever. Nearby bodies and encirclement still need escape.
            if (
                situation.EngagementStalled
                && situation.NearestEnemyDistance >= tuning.DangerRadius
                && situation.CloseEnemyCount == 0
                && situation.Encirclement < CrowdingConcern
            )
                return float.NegativeInfinity;
