    ...
    internal static partial class ProgressionBalance
    {
        ...
        )
        {
            ...
            Band(
                ...
                0f, // Rare deaths are graded across independent seeds, never forced in every run.
                MaxDeathsPerHour,
                "deaths per hour",
        ...
        {
            if (float.IsNaN(progression.Duration) || float.IsInfinity(progression.Duration))
                findings.Add("invalid duration: a finite measurement is required");
            else if (progression.Duration < MinSecondsToJudge)
                findings.Add(
        ...
        {
            if (float.IsNaN(measured) || float.IsInfinity(measured))
                findings.Add($"{name} invalid: a finite measurement is required");
