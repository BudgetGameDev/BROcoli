...
namespace BudgetGameDev.Games.Brocoli
{
    public partial class RunTelemetry
    {
        ...
        /// </summary>
        private string DescribeCaptures()
        {
            if (!_cfg.CaptureEnabled)
                return "Capture disabled (--no-capture); visual validation not performed. ";
        ...
        private bool EvaluateScenario(string reason, List<string> missing, List<string> findings)
        {
            if (!LogsAreClean(_warnings, _errors, _exceptions))
                return false;
            if (reason == "stalled" || reason == "readiness-timeout")
                return false;
        ...
        )
        {
            ...
            Bool(sb, "captureEnabled", _cfg.CaptureEnabled);
            sb.Append(',');
