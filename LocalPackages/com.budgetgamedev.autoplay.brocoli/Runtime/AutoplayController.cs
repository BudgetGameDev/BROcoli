...
namespace BudgetGameDev.Games.Brocoli
{
    ...
    /// </summary>
    public partial class AutoplayController : MonoBehaviour
    {
        ...
        internal static void InjectDiagnostics()
        {
            GameplayDiagnostics.Feature = AutoplayFeatureLog.Record;
            GameplayDiagnostics.RoomSpawned = (
                ring,
                power,
                depth,
                health,
                ...
                AutoplayScalingLog.Record(
                    new ScalingSample(ring, power, depth, health, damage, count, speed, enemies)
                );
            GameplayDiagnostics.Movement = () => BotDriver.Active ? BotDriver.Move : (Vector2?)null;
            GameplayDiagnostics.AllowCheckpoint = () => !IsActive || CheckpointsEnabled;
        }
        ...
        private void Begin()
        {
            ...
            AutoplayCaptureTriggers.Arm(
                _config.CaptureEnabled ? _config.CaptureOn : Array.Empty<string>()
            );
            ...
            {
                BeginFastForward();
                gameObject.AddComponent<AutoplayReadiness>();
            }
            ...
            ConfigureCapture(_config);

            SceneManager.sceneLoaded += OnSceneLoaded;
            EnterGame(SceneManager.LoadScene);
        }

        internal FrameCapture ConfigureCapture(AutoplayConfig config)
        {
        ...
        private void OnDestroy()
        {
            ...
            SceneManager.sceneLoaded -= OnSceneLoaded;
            GameplayDiagnostics.Feature = null;
            GameplayDiagnostics.RoomSpawned = null;
            GameplayDiagnostics.Movement = null;
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            gameObject.AddComponent<BotDriver>().ConfigureReaction(_config);
