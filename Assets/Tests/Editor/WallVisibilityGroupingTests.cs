...
public sealed class WallVisibilityGroupingTests
{
    ...
    private static int ConnectedCount(WallVisibilityWorld world, WallVisibilityWorld.Group group)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (int candidateId in group.Pieces)
            {
                if (
                    ...
                    || !DungeonWallGrouping.AreInContact(current, world.PieceOf(candidateId).Plan)
                )
                    continue;
