...
public class RunTelemetry : MonoBehaviour
{
    ...
    private int _warnings, _errors, _exceptions;
    ...
    private void OnDisable()
    {
        ...
        if (_damage != null) _damage.OnGameOver -= OnGameOver;
    }
    ...
    private void OnLog(string condition, string stackTrace, LogType type)
    {
        switch (type)
        {
            ...
            case LogType.Exception:
                _exceptions++;
                if (_firstError.Length == 0) _firstError = condition;
                break;
            default: return;
        }
    ...
    private void WriteSample()
    {
        ...
        sb.Append('{');
        Num(sb, "t", _elapsed); sb.Append(',');
        Num(sb, "x", pos.x); sb.Append(',');
        Num(sb, "y", pos.y); sb.Append(',');
        Num(sb, "hp", _stats != null ? _stats.CurrentHealth : 0f); sb.Append(',');
        Num(sb, "maxHp", _stats != null ? _stats.CurrentMaxHealth : 0f); sb.Append(',');
        Num(sb, "level", _stats != null ? _stats.CurrentLevel : 0f); sb.Append(',');
        Num(sb, "xp", _stats != null ? _stats.CurrentExperience : 0f); sb.Append(',');
        sb.Append("\"enemies\":").Append(enemies).Append(',');
        Num(sb, "nearestEnemy", nearest); sb.Append(',');
        Num(sb, "fps", fps); sb.Append(',');
    ...
    private void EndRun(string reason)
    {
        ...
        Debug.Log($"[Autoplay] Run ended ({reason}). scenario={_cfg.Scenario} passed={passed}. Out: {_cfg.OutDir}");
        Quit(passed ? 0 : 1);
    }

    private bool EvaluateScenario(string reason)
    {
        if (_exceptions > 0) return false;
        float level = _stats != null ? _stats.CurrentLevel : 0f;
        switch (_cfg.Scenario)
        {
            case "survive":  return reason == "duration";        // lived the whole run
            case "progress": return level >= _cfg.MinLevel;       // leveled up enough
            case "smoke":
            default:         return true;                         // ran without exceptions
        }
    }
    ...
    private void WriteSummary(string reason, bool passed)
    {
        ...
        sb.Append('{');
        Bool(sb, "passed", passed); sb.Append(',');
        Str(sb, "scenario", _cfg.Scenario); sb.Append(',');
        Str(sb, "reason", reason); sb.Append(',');
        sb.Append("\"seed\":").Append(_cfg.Seed).Append(',');
        Bool(sb, "deterministic", _cfg.Deterministic); sb.Append(',');
        Str(sb, "sha", _cfg.Sha); sb.Append(',');
        Num(sb, "durationSeconds", _elapsed); sb.Append(',');
        Num(sb, "finalLevel", _stats != null ? _stats.CurrentLevel : 0f); sb.Append(',');
        Num(sb, "finalHp", _stats != null ? _stats.CurrentHealth : 0f); sb.Append(',');
