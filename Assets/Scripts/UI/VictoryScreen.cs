...
public class VictoryScreen : MonoBehaviour
{
    ...
    private void CreateVictoryUI()
    {
        ...
        btnsRect.anchoredPosition = new Vector2(0, 80);
        btnsRect.sizeDelta = new Vector2(500, 60);
    ...
    private void SetupControllerNavigation()
    {
        ...
        for (int i = 0; i < menuButtons.Length; i++)
        {
            ...
            Outline outline =
                menuButtons[i].GetComponent<Outline>()
                ?? menuButtons[i].gameObject.AddComponent<Outline>();
            ...
            EventTrigger trigger =
                menuButtons[i].GetComponent<EventTrigger>()
                ?? menuButtons[i].gameObject.AddComponent<EventTrigger>();
    ...
    private void HandleControllerNavigation()
    {
        ...
        if (Mathf.Abs(nav) > 0.1f)
        {
            ...
            int newIndex = Mathf.Clamp(
                selectedIndex + (int)Mathf.Sign(nav),
                0,
        ...
        if (Gamepad.current != null)
            select |= Gamepad.current.buttonSouth.wasPressedThisFrame;
