...
public class ControllerUINavigator : MonoBehaviour
{
    ...
    [SerializeField]
    private Color selectedBorderColor = new Color(1f, 0.9f, 0.3f, 1f);

    [SerializeField]
    private float selectedBorderWidth = 6f;
    ...
    [SerializeField]
    private float inputRepeatDelay = 0.3f;
    ...
    private Dictionary<Button, SelectionVisual> buttonVisuals =
        new Dictionary<Button, SelectionVisual>();
    ...
    private void SetupButtonVisual(Button btn)
    {
        ...
        var visual = new SelectionVisual { originalScale = rt.localScale, rectTransform = rt };
    ...
    void Update()
    {
        if (!isActive || navigableButtons.Count == 0)
            return;
    ...
    {
        if (Time.unscaledTime - lastInputTime < inputRepeatDelay)
            return;
        ...
        if (Gamepad.current != null)
        {
            ...
            if (Mathf.Abs(dpad.x) > 0.5f)
                nav.x = Mathf.Sign(dpad.x);
            else if (Mathf.Abs(stick.x) > 0.5f)
                nav.x = Mathf.Sign(stick.x);

            if (Mathf.Abs(dpad.y) > 0.5f)
                nav.y = Mathf.Sign(dpad.y);
            else if (Mathf.Abs(stick.y) > 0.5f)
                nav.y = Mathf.Sign(stick.y);
        }
    ...
    private void UpdateVisuals()
    {
        foreach (var kvp in buttonVisuals)
        {
            ...
            if (btn == null || visual.rectTransform == null)
                continue;
