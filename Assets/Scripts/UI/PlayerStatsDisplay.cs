...
public class PlayerStatsDisplay : MonoBehaviour
{
    ...
    private float baseDamage = PlayerStats.DefaultBaseDamage;
    ...
    public void RefreshStats()
    {
        ...
        if (playerStats == null || statsText == null)
        {
            ...
            {
                if (statsText != null)
                    statsText.text = "No player stats found";
                return;
            }
        }
        ...
        sb.AppendLine(
            FormatStat("Speed", playerStats.CurrentMovementSpeed, baseSpeed, false, false, "", 1)
        );
        sb.AppendLine(
            FormatStat("Atk Speed", playerStats.CurrentAttackSpeed, baseAttackSpeed, true)
        ); // Lower is better
        ...
        sb.AppendLine(
            FormatStat("Crit %", playerStats.CurrentCritChance, baseCritChance, false, false, "%")
        );
        sb.AppendLine(
            FormatStat(
                "Crit DMG",
        ...
        sb.AppendLine(
            FormatStat("Dodge", playerStats.CurrentDodgeChance, baseDodge, false, false, "%")
        );
        sb.AppendLine(FormatStat("Armor", playerStats.CurrentArmor, baseArmor));
        sb.AppendLine(
            FormatStat("Regen", playerStats.CurrentHealthRegen, baseRegen, false, false, "/s", 1)
        );
        sb.AppendLine(
            FormatStat("Lifesteal", playerStats.CurrentLifeSteal, baseLifeSteal, false, false, "%")
        );
    ...
    )
    {
        ...
        if (lowerIsBetter)
            diff = -diff; // Invert for stats where lower is better
