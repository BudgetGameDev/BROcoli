...
public sealed class WallVisibilityStateMachine
{
    ...
    {
        if (selection.TryGetValue(groupId, out OcclusionTargetKind existing) && existing <= cause)
            return;
    ...
    public void EndFrame(float time)
    {
        ...
        foreach (KeyValuePair<int, Entry> pair in entries)
        {
            ...
            else if (!selected && time - entry.LastSelectedTime > settings.FlickerReacquireWindow)
